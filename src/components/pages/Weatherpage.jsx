...
const GwangjuWeather = () => {
  ...
  useEffect(() => {
    ...
    const fetchWeather = async () => {
      ...
      if (!API_KEY) {
        setError('환경 변수에 OpenWeatherMap API 키가 설정되지 않았습니다. .env 파일에 VITE_OPENWEATHER_API_KEY를 추가해주세요.');
        setLoading(false);
        return;
      }
  ...
  const getWeatherInfo = () => {
    ...
    return {
      ...
      sunrise: new Date(weatherData.sys.sunrise * 1000).toLocaleTimeString('ko-KR', {hour: '2-digit', minute: '2-digit'}),
      sunset: new Date(weatherData.sys.sunset * 1000).toLocaleTimeString('ko-KR', {hour: '2-digit', minute: '2-digit'})
    };
  };
  ...
  if (loading) {
    return (
      <div style={{
        ...
        <div style={{ fontSize: '24px', fontWeight: '300' }}>날씨 정보를 불러오는 중...</div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div style={{
        ...
        <div style={{ fontSize: '20px', lineHeight: '1.6', maxWidth: '600px' }}>{error}</div>
      </div>
    );
  }
